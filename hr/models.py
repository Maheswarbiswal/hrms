...
class Employee(models.Model):
    ...
    biometric_id = models.CharField(max_length=100, blank=True, null=True, unique=True, help_text="ID from the biometric machine")
    ...
    # Remove the old single manager fields (or keep for backward compatibility)
    ...
    # Add new JSON field for multiple managers
    reporting_managers = models.JSONField(default=list, blank=True, null=True)
    ...
    @property
    def reporting_manager_ids(self):
        """Get list of reporting manager IDs"""
        if self.reporting_managers:
            return [mgr.get('id') for mgr in self.reporting_managers if mgr.get('id')]
        elif self.reporting_manager_id:
            return [self.reporting_manager_id]
        return []
    
    @property
    def reporting_manager_names(self):
        """Get list of reporting manager names"""
        if self.reporting_managers:
            return [mgr.get('name') for mgr in self.reporting_managers if mgr.get('name')]
        elif self.reporting_manager:
