...
class AutoLeaveBalanceService:
    ...
    def initialize_employee_leave_balance(employee):
        """
        Initialize leave balance when a new employee is created- USING FINANCIAL YEAR
        ...
        print(f"DEBUG: Initializing for {employee.first_name} - Financial Year: {fy_start}-{fy_end}")
        print(f"DEBUG: Joining date: {employee.date_of_joining}")
        ...
        if annual_leave:
            ...
            if created:
                ...
                fy_start_date = date(fy_start, 4, 1)  # April 1st of financial year
                
                if employee.date_of_joining and employee.date_of_joining < fy_start_date:
                ...
                if is_on_probation:
                    ...
                    if employee.date_of_joining and employee.date_of_joining < fy_start_date:
                        ...
                        if current_month >= 4:
                            months_in_current_fy = current_month - 3  # April=1, May=2, etc.
                        else:
                            # Jan-Mar are month 10,11,12 of financial year
                            months_in_current_fy = current_month + 9  # Jan=10, Feb=11, Mar=12
                        
                        months_in_current_year = months_in_current_fy
                    else:
    ...
    def monthly_accrual_cron():
        ...
        return updated_count  # ✅ FIXED: outside the for loop — runs after ALL employees processed
